...
function obtenerTemperaturasInvierno($matrizTemp){
    //int $fila, $col, $filaIndice, $columnaIndice
    //array $temperaturasInvierno
    $filaIndice = 0;
    $columnaIndice = 0; 

    $temperaturasInvierno = [];

    for ($fila = 5; $fila < 10; $fila++){
        for ($col = 6; $col < 8; $col++){
...
do{
    mostrarMenu();
	$opcion = trim(fgets(STDIN)); //como validar??
	switch ($opcion){
        case 1: 
            echo "Se realizará la carga automática de temperaturas desde el año 2014 al 2023");
            $cargada <- true;
            echo "Los datos de las temperaturas fueron cargadas con éxito";
            break;
        case 2:                  
            echo "A continuación deberá cargar las temperaturas desde Enero a Diciembre; desde el 2014 al 2023)";
	        $matrizTemperatura = cargaManual($meses, $arregloAnios);
            $cargada = true;
            echo "Los datos de las temperaturas fueron cargados con éxito";
            break;
		case 3: 
            if($cargada == false){
                echo "Todavía no hay temperaturas cargadas. Por favor cargue las temperaturas de forma automática o manual antes de intentar mostrarlas";
            } else {
                mostrarTemperaturas($matrizTemperatura, $meses, $anios);
            }
            break;
        case 4: 
            if($cargada == false){
	            ...
	            do{
                   echo "Ingrese un año válido";
                   $anioIngresado = trim(fgets(STDIN));
                   $indiceAnio <- validarAnio($anios, $anioIngresado);
                   if($indiceAnio == -1){
		               echo "El  año ingresado no es  válido. Intente de nuevo"; 
                   }
                } while ($indiceAnio >= 0);
                do{
                    echo "Ingrese el número del mes (1- Enero, 2- Febrero, …, 12- Diciembre";
                    $mesUsuario = trim(fgets(STDIN));
                    $indiceMes <- validarMes($anios, $mesUsuario);
                    if($indiceMes == -1){
                        echo "El mes ingresado no es válido. Debe ingresar entre 1 y 12. Intente de nuevo"; 
                    }
                } while ($indiceMes >= 0);
                $temperatura <- retornaTemperaturaPorFecha($matrizTemperatura,$indiceAño, $indiceMes )
	            echo "La temperatura registrada para el año ". $anios[$indiceAnio]. " mes ". $meses[$indiceMes]. "" es "." $temperatura; 
            }
            break;
        case 5:
            if($cargada == false){
	            ...
	            $temperaturaPorAnio = obtenerTemperaturasPorAnio($matrizTemperaturas, $indiceAnio)
                mostrarTemperaturas($temperaturaPorAnio, $meses, $anios);   	   
            }
            break;
        case 6:
            if($cargada == false){
                ...
                $temperaturasPorMes = obtenerTemperaturasPorMes($matrizTemperatura, $indiceMes)
                //$anioElegido = [indiceMes] 
                mostrarTemperaturas($temperaturaPorMes, $meses, $anio);
                $promedio = promedioDelMes($temperaturasPorMes);
                echo "El promedio de temperaturas para el año elegido es ". $promedio;
            }
            break;
        case 7:
            if($cargada == false){
                ...
                $arregloMaxima = hallarTemperaturaMax($matrizTemp, $meses, $anios);
                $arregloMinima = hallarTemperaturaMin($matrizTemp, $meses, $anios);

                //El arreglo en la posición 0 guarda la temperatura, en la posición 1 el índice del año
                //y en la posición 2 el índice del mes
                echo "Registro de la temperatura mínima";
                echo "Año: ". $anios[$arregloMinima[1]]. " Mes: ". $meses[$arregloMinima[2]]. " Temperatura mínima: ". $arregloMinima[0];
                echo "Registro de la temperatura máxima";
                echo "Año: ". $anios[$arregloMaxima[1]]. " Mes: ". $meses[$arregloMaxima[2]]. " Temperatura maxima: ". $arregloMaxima[0];
            }
            break;
        case 0:
            echo "Saliendo ...";
            break;
        default:
            echo "Opción no válida. Por favor ingrese una opción entre 1 y 10";
            break;
    }
} while($opcion <> 0);
